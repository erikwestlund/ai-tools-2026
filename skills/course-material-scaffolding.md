...
- Include a small `.Rproj` file and `.code-workspace` file for demo folders only when the instructor will control the edits. Student-editable notebook work should be scaffolded under `practice/tasks/` and copied to `practice/work/` by `updater.R`.
...
- Put practice starter project folders under `practice/tasks/<task-name>/`.
...
- Student-facing instructions should tell students to open copied folders under `practice/work/`, not directories under `modules/` or `practice/tasks/`.
